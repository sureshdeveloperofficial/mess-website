...
const Features = () => {
  ...
  return (
    <section id='features' className='py-16 bg-white relative overflow-hidden'>
        ...
        >
          {FEATURE_DATA.map((item, i) => (
            <motion.div
              ...
              className='feature-card p-10 relative rounded-[3.5rem] bg-[#FFF9E6] border border-[#FFD54F]/20 shadow-xl shadow-[#FFD54F]/5 flex flex-col items-center text-center group transition-colors duration-500 hover:bg-white'
            >
              <div className='w-20 h-20 bg-[#FFD54F]/20 rounded-3xl flex items-center justify-center mb-8 group-hover:scale-110 group-hover:bg-[#FFD54F] transition-all duration-500'>
                <Icon icon={item.icon} className='text-4xl text-amber-600 group-hover:text-grey-dark transition-colors duration-500' />
              </div>

              <h3 className='text-xl font-extrabold text-grey-dark mb-4 tracking-tight group-hover:text-amber-600 transition-colors'>
                {item.heading}
              </h3>
              ...
              <div className='absolute bottom-6 right-8 opacity-0 group-hover:opacity-100 transition-opacity duration-500'>
                <Icon icon='ion:sparkles-outline' className='text-2xl text-[#FFD54F]/30' />
              </div>
            </motion.div>
          ))}
        </motion.div>
      ...
      {/* Background accents */}
      <div className='absolute top-1/2 left-0 w-64 h-64 bg-[#FFD54F]/10 rounded-full blur-[100px] pointer-events-none'></div>
      <div className='absolute bottom-0 right-0 w-96 h-96 bg-[#FFD54F]/5 rounded-full blur-[120px] pointer-events-none'></div>
    </section>
  )
}
